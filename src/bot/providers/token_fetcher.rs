use crate::bot::contracts::IERC20;
use crate::bot::models::token::{Token, TokenRegistry};
...
pub async fn get_or_fetch_token(
    ...
    if let Some(token) = registry.get_token(address) {
        return Ok((address, token.decimals));
    }
    ...
    Ok((address, token_data.decimals))
}
